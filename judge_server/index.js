...
//         // --- START DEBUG LOGS: Verify code written to file ---
//         const writtenCode = await fs.readFile(path.join(submissionTempDir, filename), 'utf8');
//         console.log(`DEBUG: Code written to ${filename}:\n${writtenCode.substring(0, 200)}...`); // Log first 200 chars if long
//         // --- END DEBUG LOGS ---
...
//                 // --- START DEBUG LOGS: Compilation command ---
//                 console.log(`DEBUG: Docker Compile Command: ${dockerCompileCommand}`);
//                 // --- END DEBUG LOGS ---
...
//             // --- START DEBUG LOGS: Run command ---
//             console.log(`DEBUG: Docker Run Command for Test Case ${i + 1}: ${dockerRunCmd}`);
//             // --- END DEBUG LOGS ---
...
//                 // --- START DEBUG LOGS: Read outputs on error ---
//                 console.log(`DEBUG: Test Case ${i + 1} (Error) - Raw actualOutput from file: '${actualOutput}'`);
//                 console.log(`DEBUG: Test Case ${i + 1} (Error) - Raw stderrOutput from file: '${stderrOutput}'`);
//                 // --- END DEBUG LOGS ---
...
//                     passed: passedThisTestCase, // Boolean flag for frontend
//                     message: message,
...
// NEW HELPER: Safely read file content and ensure it's a trimmed string
const getFileContent = async (filePath) => {
    try {
        const content = await fs.readFile(filePath, 'utf8');
        return content.trim();
    } catch (e) {
        // If file doesn't exist or cannot be read, return empty string and log a warning
        console.warn(`Warning: Could not read file ${filePath}. Error: ${e.message}`);
        return '';
    }
};
    ...
    try {
        ...
        if (compileCommand) {
            try {
                ...
                await executeShellCommand(dockerCompileCommand);
                compilerOutput = await getFileContent(path.join(submissionTempDir, 'compiler_errors.txt')); // Use helper
                ...
                console.log('Judge Server: Final testCasesPassedCount (Compilation Error Catch):', 0);
                 return res.json({
                    ...
                    status: submissionStatus,
                    verdict: overallVerdict,
                    compilerOutput: compilerOutput, // Already trimmed by helper if from file, or direct error message
                    detail: 'Compilation failed due to an internal error or syntax issues.',
                    testCasesPassed: 0,
                    totalTestCases: testCases.length,
                    detailedResults: detailedResults
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            try {
                ...
                actualOutput = await getFileContent(outputFilePath); // <--- MODIFIED
                stderrOutput = await getFileContent(errorFilePath); // <--- MODIFIED
